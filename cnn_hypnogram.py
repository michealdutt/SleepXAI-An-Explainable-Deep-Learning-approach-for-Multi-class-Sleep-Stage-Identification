...
print(train_ids)
print(test_ids)
train_val, test = [x for x in files if x.split("/")[-1][:5] in train_ids], [x for x in files if x.split("/")[-1][:5] in test_ids]
...
for record in tqdm(test_dict):
    ...
    plt.legend()
    plt.show()
    plt.savefig(str(test_ids)+"original_sleep_stage_CNN.png")
