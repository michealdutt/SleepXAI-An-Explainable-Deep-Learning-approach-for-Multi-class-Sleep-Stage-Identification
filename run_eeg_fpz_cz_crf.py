...
def train_model(train_files, model_save_path):
    ...
    checkpoint = ModelCheckpoint(model_save_path, monitor='val_crf_viterbi_accuracy', verbose=1, save_best_only=True,
                                 mode='max')
...
def cross_validation_training():
    ...
    for i in subject_ids_1:
        ...
        gt, preds = eval_model(load_model(model_save_path_1, custom_objects={'CRF': CRF,
                                                                             'crf_loss': crf_loss,
    ...
    eeg_fpz_cz_report = pd.DataFrame(
        classification_report(y_true=allgt_1, y_pred=allpreds_1, output_dict=True)).transpose()
    eeg_fpz_cz_report.to_csv('eeg_fpz_cz_report_crf.csv', index=True)
    ...
    eeg_fpz_cz_confusion_report.to_csv('eeg_fpz_cz_confusion_report_crf.csv', index=True)
    accuracy_fpz.append(acc_1)
    accuracy_fpz = np.array(accuracy_fpz)
    np.savetxt("accuracy_fpz_crf.csv", accuracy_fpz, delimiter=",")
