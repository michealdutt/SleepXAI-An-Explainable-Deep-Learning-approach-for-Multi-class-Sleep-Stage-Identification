...
def cross_validation_training():
    ...
    model_save_path_2 = "pz_oz_crf.h5"
    ...
    for i in subject_ids_2:
        ...
        gt, preds = eval_model(load_model(model_save_path_2, custom_objects={'CRF': CRF,
                                                                             'crf_loss': crf_loss,
    ...
    eeg_pz_oz_report = pd.DataFrame(
        classification_report(y_true=allgt_2, y_pred=allpreds_2, output_dict=True)).transpose()
    eeg_pz_oz_report.to_csv('eeg_pz_oz_report_crf.csv', index=True)
    ...
    eeg_pz_oz_confusion_report.to_csv('eeg_pz_oz_confusion_report_crf.csv', index=True)
    accuracy_oz.append(acc_2)
    accuracy_oz = np.array(accuracy_oz)
    np.savetxt("accuracy_oz_crf.csv", accuracy_oz, delimiter=",")
